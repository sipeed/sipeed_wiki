...
相较于上面我们简单了介绍 MaixPy IDE 以及串口的的使用方法，在实际运行时我们更希望代码保存在文件系统，这样不需要再次敲代码可直接运行程序更为便利快捷，小伙伴们可以参考以下提供的示例方法进行操作。
...
前文我们接触到了如何打开编辑器并直接编辑文件系统中的文件，但如果当代码量庞大或需要高亮支持时就不适用了，我们需要在电脑上写完放入开发板的文件系统内。
...
>这篇文档完全是按新手小白的使用步骤一步一步来写的，在【更多功能应用】里编写的是 MaixPy 的应用案例以及使用方式，如果是没有接触过 `Python 以及 MicroPython` 的小伙伴们一定要先学会基础的语法知识，切记！没有基础的小伙伴不可以跳过上文！！！更多功能应用是基于上文的入门的基础下写的，可以搭配 API 文档和[MaixPy 例程仓库](https://github.com/sipeed/MaixPy_scripts)一同学习。

更多功能应用分为五大部分：`系统基础功能`、`外设模块拓展`、`图像处理基础（image）`、`传统算法应用`、`神经网络应用（AI）`.

**更多功能应用：**[点击查看](https://wiki.sipeed.com/soft/maixpy/zh/course/index.html)

### 如何进行配置开发板

在使用外设模块拓展的时候，很多的小伙伴跑例程会报错极多的片上外设示例因不同的板卡配置不同需要进行相对应手动配置才能使用。
