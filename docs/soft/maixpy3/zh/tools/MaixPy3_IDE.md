...
为了节约用户自己配置的操作，我们提供了开箱即用的绿色软件，它是借助 pyinstaller 将 ipython 、 jupyter 、 rpyc_ikernel 合并导出的 Python 程序，你可以在[下载站](https://dl.sipeed.com/shareURL/MaixII/MaixPy3-IDE)中获得 MaixPy3 IDE exe 安装包。成功安装程序之后，它会调用系统默认的浏览器以网页的形式打开。所使用的浏览器应该为 Chromium 内核，比如 [Microsoft Edge](https://www.microsoft.com/zh-cn/edge) 和 Chrome。
...
1. 确认使用最新镜像系统，其中的 maixpy3 大于 0.4.0 版本，要准备 烧录好系统的SD 卡。
2. 确认使用最新的 IDE 工具，其版本号应大于 0.4.0 版本，安装软件时安装所有提示需要安装的驱动。
3. 接上 M2DOCK OTG 口，屏幕显示画面，电脑产生 U 盘等行为都表示系统已经成功启动。
