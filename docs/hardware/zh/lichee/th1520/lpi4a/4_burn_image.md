...
注意不同版本的硬件进入烧录模式的方式略有差异，请参考后面的章节。

> **外壳提示：**RESET 和 BOOT 是两个独立的按键。部分 LPi4A 外壳会露出 RESET 按键，而把相邻的 BOOT 按键挡住。如果够不到 BOOT 按键，请在执行 USB 烧录步骤前先部分拆开或取下外壳，以便按到该按键。只按 RESET 无法进入 USB 烧录模式。
...
除了像上面那样按住 BOOT 按键外，还要注意检查底板的拨码开关是否为 EMMC 启动模式：  
