...
主界面会显示网络状态、设备 IP 地址、当前视频分辨率、帧率，以及键盘和鼠标输入状态。完成网络配置后，可在此处查看 NanoKVM Go 当前获得的 IP 地址。

分辨率和帧率右侧的键盘区域用于显示键盘输入状态，上方显示特殊键，例如 Cmd、Ctrl；下方显示普通键，例如 C、A。下图示例 Cmd 和 C 同时显示，表示 NanoKVM Go 正在进行一组 Cmd+C 键盘输入状态。

最右侧的鼠标箭头用于表示鼠标输入状态，控制鼠标时，箭头会亮起。
...
点击悬浮栏中的鼠标设置图标，可调整远程控制时的鼠标输入方式、输入校准和 HID 相关功能。新手用户建议优先保持默认设置；当鼠标无法控制、指针位置不准确或滚轮方向不符合习惯时，再按需修改以下选项。

+ 鼠标模式用于选择鼠标输入的传输方式，需要按被控设备的类型选择：

    - `跟随式鼠标`：适合电脑等普通桌面系统，被控端指针会跟随网页端指针移动。被控设备为 Android 手机时请勿使用该模式，否则无法正常控制。
    - `跟随式鼠标（更兼容 Android）`：被控设备为 Android 手机时使用，适配 Android 的指针和触控输入，兼容性更好。
    - `多点触摸屏`：模拟多点触摸屏，适合使用手机作为控制设备来操控另一台手机，可进行更接近触摸屏的操作。
    - `独占式鼠标`：发送相对位移，适合游戏、远程桌面、BIOS/UEFI 等需要相对位移输入或锁定鼠标的场景。选中后需要点击画面锁定指针，本地鼠标指针会被隐藏。

![NanoKVM Go 鼠标设置菜单](./../../../assets/NanoKVM/go/user_guide/nanokvm-go-mouse-mode.webp)

+ 输入校准用于管理各跟随式（绝对坐标）输入模式的映射配置。内置的 `无校准` 和 `自动校准` 为只读，其中 `自动校准` 会跟随当前去黑边结果动态更新；若指针位置仍有偏差，可新建自定义配置后执行四点校准。校准前需要先关闭去黑边，且依赖被控端可见的指针或触点反馈。

+ 滚轮方向用于切换滚轮滚动方向，可根据个人习惯选择向上或向下。滚轮速度用于调整滚动灵敏度，网页滚动过快时可调慢，滚动距离不足时可调快。`多点触摸屏` 模式下不提供这两个选项。
...
+ `修复 iPhone 拖拽` 用于处理 iPhone 浏览器中可能出现的拖拽异常。`指针输入指南` 可查看在触屏设备上滑动移动、点按单击、按住拖动等操作说明。

![NanoKVM Go 修复 iPhone 拖拽](./../../../assets/NanoKVM/go/user_guide/nanokvm-go-fix-iphone-drag.webp)
...
镜像挂载用于将镜像文件挂载给被控设备，常用于系统安装、系统维护或启动盘测试等场景。使用前请确认镜像文件来源可靠，并根据被控设备的启动顺序选择对应的启动项。详细使用方法请参考 [NanoKVM Cube 用户指南的镜像挂载章节](../NanoKVM/user_guide.html#ISO镜像挂载以及远程装机)。
