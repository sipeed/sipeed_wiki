...
如果 NanoKVM Go 上电后进入反复重启（屏幕反复亮起再熄灭，或始终进不了主界面），一般是供电不足或供电不稳导致的。

以下接法容易触发该问题：

1. 用电脑的 USB 口供电：USB-A 口或普通 USB-C 数据口通常只保证 5 V / 0.5 A，不足以支撑供电需求。
2. 使用输出功率过小的充电头：例如老式 5 V / 1 A 充电头，带载后长期处于满载甚至过载状态。
3. 经由扩展坞，USB Hub 或多口充电器供电：电流要与其他设备分摊，带载后电压不稳。
4. 使用劣质、过长或经过多次转接的线缆：电阻偏大，到达设备端的电压已经不够。

**解决办法：**准备一套支持 USB-PD 的充电器和线缆，单独为 NanoKVM Go 供电。

### 供电充足时仍然一直重启

如果已经使用支持 USB-PD 的充电器和线缆单独为 NanoKVM Go 供电（参考上一节），设备上电后仍然反复重启，屏幕反复亮起再熄灭或者始终进不了主界面，请重新烧录 NanoKVM Go 镜像。

具体步骤请参考[烧录镜像](./system/flashing.html)。

**说明：**

- 重新烧录会清除设备内的所有配置，请在操作前确认没有需要保留的数据。
- 如果重新烧录**最新**镜像后仍然反复重启，请按照文末的[反馈方式](#反馈方式)联系我们。

### 同时插着两个 USB-C 时，拔掉辅助 USB-C 接口的 PD 电源后设备会重启

如果同时连接了数据 USB-C 接口和**辅助 USB-C 接口（闪电图标，Power Port）**，在运行过程中拔掉辅助 USB-C 接口上的 USB-PD 电源，NanoKVM Go 会出现一次**重启**。**这属于正常现象**，并非设备故障。

![辅助 USB-C 接口（闪电图标，Power Port）位置示意](./../../../assets/NanoKVM/go/faq/nanokvm-go-power-port-pd-unplug-reboot.webp)

**原因：**接入辅助 USB-C 接口的 PD 电源后，NanoKVM Go 会优先由该接口取电；拔掉 PD 电源时，设备需要把供电来源切换回被控设备的 USB-C 接口。切换供电来源的过程中会出现一次短暂的断电，因此设备会重启。

**说明：**

- 该重启通常只发生一次，之后设备会继续由被控设备的 USB-C 接口供电，无需额外处理。
- 如果拔掉后设备不是重启一次，而是**反复重启**，说明被控设备的 USB-C 接口供电能力不足，请参考[上电后一直重启](#上电后一直重启)进行处理。
